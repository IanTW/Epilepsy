...
yax <- list(
  title = "", tickfont = list(size =11))
xax <- list(
  title = "Number of Times Selected")

Feats <- dat1$Feats
Counts <- dat1$Sum
dat1 <- data.frame(Feats, Counts, stringsAsFactors = FALSE)
dat1$Feats <- factor(dat1$Feats, levels = unique(dat1$Feats)[order(dat1$Counts, decreasing = FALSE)])
p1 <- plot_ly(dat1, x = ~Counts, y = ~Feats, type = "bar", orientation = 'h', name= "Statistics") %>%
...
dat2 <- z[,.(Sum = sum(Freq)), by = .(Feats)]

#Rename factors
dat2$Feats <- revalue(dat2$Feats, c("Alpha_Rel" = "Alpha_Norm",
...
Feats <- dat2$Feats
Counts <- dat2$Sum
dat2 <- data.frame(Feats, Counts, stringsAsFactors = FALSE)
dat2$Feats <- factor(dat2$Feats, levels = unique(dat2$Feats)[order(dat2$Counts, decreasing = FALSE)])
p2 <- plot_ly(dat2, x = ~Counts, y = ~Feats, type = "bar", orientation = 'h', name = "Spectral") %>%
  layout(margin = m, yaxis = yax, xaxis = xax)
...
Feats <- dat3$Feats
Counts <- dat3$Sum
dat3 <- data.frame(Feats, Counts, stringsAsFactors = FALSE)
dat3$Feats <- factor(dat3$Feats, levels = unique(dat3$Feats)[order(dat3$Counts, decreasing = FALSE)])
p3 <- plot_ly(dat3, x = ~Counts, y = ~Feats, type = "bar", orientation = 'h', name = "Combined") %>%
  layout(margin = m, yaxis = yax, xaxis = xax)

subplot(p1,p2,p3, nrows = 3, shareX = TRUE, heights = c(0.2, 0.4, 0.4)) %>%  layout(legend = list(x=0.6,y=0.1))
